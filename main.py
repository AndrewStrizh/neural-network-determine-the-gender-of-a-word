...
def train_more(model, epochs, words, classes, speed):
    classes = torch.tensor(classes)
    for epoch in range(epochs + 1):
        words_batch, classes_batch = gen_batch(words, classes)
        loss = model.train(words_batch, classes_batch, speed)
        if epoch % (epochs // 10) == 0:
            print(f'completed {epoch % (epochs // 10)}%', 'loss = ', loss)
    torch.save(model, 'bumba.txt')
